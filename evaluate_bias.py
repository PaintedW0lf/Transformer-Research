...
def evaluate_models(western_path: str, eastern_path: str, output_dir: str, max_tokens: int = 150, temperature: float = 0.6, top_p: float = 0.9, top_k: int = 50, repetition_penalty: float = 1.15):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.6,
        help="Temperature for text generation (lower = less creative, higher = more diverse)",
    )
    parser.add_argument(
        "--top-p",
        type=float,
        default=0.9,
        help="Nucleus sampling threshold (lower = more focused)",
    )
    parser.add_argument(
        "--top-k",
        type=int,
        default=50,
        help="Top-k sampling (limits vocabulary to k most likely tokens)",
    )
    parser.add_argument(
        "--repetition-penalty",
        type=float,
        default=1.15,
        help="Penalty for repeated tokens (1.0 = no penalty, >1.0 = reduces repetition)",
    )
    parser.add_argument(
        "--analyze-only",
        action="store_true",
